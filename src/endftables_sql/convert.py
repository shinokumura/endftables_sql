...
from endftables_sql.models import Endf_Reactions, Endf_XS_Data, Endf_Residual_Data, Endf_N_Residual_Data, Endf_FY_Data, Endf_Angle_Data
from endftables_sql.config import session, connection,  MT_PATH_JSON, LIB_LIST, LIB_PATH
from endftables_sql.submodules.utilities.elem import ELEMS, ztoelem
...
def read_libs(p):
    ...
    if p != "fy":
        ...
        types = ["xs", "residual", "angle"] # and "angle"

    else:
        nuclides = [
            d
            for d in os.listdir(os.path.join(LIB_PATH, "FY", p))
            if os.path.isdir(os.path.join(LIB_PATH, "FY", p, d))
        ]
        types = ["fy"]
            ...
            for type in types:
                ...
                if type == "fy":
                    table_path = os.path.join(
                        LIB_PATH, "FY", p, nuclide, lib, "tables", "FY"
                    )
                ...
                print("Processing: ", p, nuclide, lib, type)
                ...
                for file in files:
                    ...
                    if type == "xs":
                        ...
                        mt = re.sub(r"\D", "", name[2])
                        residual = None
                        # en1 = re.sub(r'\D', '', name[3]).replace('Eang','')
                        # en2 = re.sub(r'\D', '', name[4])
                        # if en1 == "2.5E":
                        #     e_inc = 2.5E-8
                        # else:
                    ...
                    elif check(p, nuclide, lib, type, mt, residual) > 0 and type != "fy":
                        print(p, nuclide, lib, type, mt, residual, "exist")
                        continue
...
def create_libdf_angle(libfile, reaction_id):
    with open(libfile, "r") as f:
        en_inc = float(f.readlines()[18][23:36].strip())
